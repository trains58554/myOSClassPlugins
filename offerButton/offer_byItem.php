<?php if(osc_is_web_user_logged_in() ) { 
    ...
    Search::newInstance()->addConditions(sprintf("%st_offer_button.seller_id = %d", DB_TABLE_PREFIX, $i_userId));
                    ...
                    <?php } else { ?>
                        <?php while(osc_has_items()) { ?>
                                <div class="item" >
                                        ...
                                        <?php 
                                        $conn   = getConnection();
                                        $offers = $conn->osc_dbFetchResults("SELECT * FROM %st_offer_button WHERE item_id  = '%d' ORDER BY id DESC", DB_TABLE_PREFIX, osc_item_id()); ?>
                                        ...
                                        <table cellpadding="0" cellspacing="0" border="0" class="display" id="datatables_list">
                                        	...
                                        	<?php $user = User::newInstance()->findByPrimaryKey($userOffer['user_id']); ?>
                                        		...
                                        		<td><?php echo $user['s_name']; ?><div><?php if(($userOffer['offer_status'] == 2) || ($userOffer['offer_status'] == 3)){?><a href="<?php echo osc_base_url(true).'?page=custom&file=offerButton/offer_edit.php&offer-action=accept&offer-id='.$userOffer['id'] .'&user-id=' . $userOffer['user_id'] . '&item-id=' . $userOffer['item_id']; ?>"><?php }?><?php _e('Accept', 'offer_button'); ?><?php if(($userOffer['offer_status'] == 2) || ($userOffer['offer_status'] == 3)){?></a><?php }?> | <?php if(($userOffer['offer_status'] == 1) || ($userOffer['offer_status'] == 3)){?><a href="<?php echo osc_base_url(true).'?page=custom&file=offerButton/offer_edit.php&offer-action=pending&offer-id='.$userOffer['id'] .'&user-id=' . $userOffer['user_id'] . '&item-id=' . $userOffer['item_id']; ?>"><?php }?><?php _e('Pending', 'offer_button'); ?><?php if(($userOffer['offer_status'] == 1) || ($userOffer['offer_status'] == 3)){?></a><?php }?> | <?php if(($userOffer['offer_status'] == 1) || ($userOffer['offer_status'] == 2)){?><a href="<?php echo osc_base_url(true).'?page=custom&file=offerButton/offer_edit.php&offer-action=declined&offer-id='.$userOffer['id'] .'&user-id=' . $userOffer['user_id'] . '&item-id=' . $userOffer['item_id']; ?>"><?php }?><?php _e('Decline', 'offer_button'); ?><?php if(($userOffer['offer_status'] == 1) || ($userOffer['offer_status'] == 2)){?></a><?php }?></div></td>
