    		...
    		if (!$offerAccepted) {
    			...
    			if($offerCheck){
    				if($offerCheck['offer_status'] == 2){
    					...
    					if(($offerCheck['offer_status'] = 3) && ($userOffer < $offerCheck['offer_value'])){
    						...
    						//adds offer to item from user 
                				...
                				$message = __('Your new offer of '. $userOffer . ' ' . $item_idA['fk_c_currency_code'] . ' has been submitted' ,'offer_button');
    				...
    				//adds offer to item from user 
                		$conn->osc_dbExec("INSERT INTO %st_offer_button (user_id, seller_id, item_id, offer_value, offer_status) VALUES ('%d', '%d', '%d', '%f', '%d')", DB_TABLE_PREFIX, osc_logged_user_id(), $sellerId, $itemId, $userOffer, '2');
                		offer_button_send_email($item_idA, $userOffer);
                		$message = __('Your offer of ' . $userOffer . ' ' . $item_idA['fk_c_currency_code'] . $locked['locked'] . ' has been submitted','offer_button');
    			}
    			...
    			if($offerCheck){
    				if($offerCheck['offer_status'] == 2){
    					...
    					if(($offerCheck['offer_status'] = 3) && ($userOffer < $offerCheck['offer_value'])){
    						...
    						//adds offer to item from user 
                				$conn->osc_dbExec("INSERT INTO %st_offer_button (b_email, b_name, seller_id, item_id, offer_value, offer_status) VALUES ('%s', '%s', '%d', '%d', '%f', '%d')", DB_TABLE_PREFIX, $bEmail, $bName, $sellerId, $itemId, $userOffer, '2');
    				...
    				//adds offer to item from user 
                		$conn->osc_dbExec("INSERT INTO %st_offer_button (b_email, b_name, seller_id, item_id, offer_value, offer_status) VALUES ('%s', '%s', '%d', '%d', '%f', '%d')", DB_TABLE_PREFIX, $bEmail, $bName, $sellerId, $itemId, $userOffer, '2');
                		offer_button_send_email($item_idA, $userOffer);
                		$message = __('Your offer of ' . $userOffer . ' ' . $item_idA['fk_c_currency_code'] .' has been submitted','offer_button');
    			}
