                                ...
                                <div class="item" >
                                        ...
                                        <table cellpadding="0" cellspacing="0" border="0" class="display" id="datatables_list">
                                        	...
                                        	<?php $user = User::newInstance()->findByPrimaryKey($userOffer['user_id']); ?>
                                        		...
                                        		<td><?php echo $user['s_name']; ?><div><?php if(($userOffer['offer_status'] == 2) || ($userOffer['offer_status'] == 3)){?><a href="<?php echo osc_base_url(true).'?page=custom&file=offer_button/offer_edit.php&offer-action=accept&offer-id='.$userOffer['id'] .'&user-id=' . $userOffer['user_id']; ?>"><?php }?><?php _e('Accept', 'offer_button'); ?><?php if(($userOffer['offer_status'] == 2) || ($userOffer['offer_status'] == 3)){?></a><?php }?> | <?php if(($userOffer['offer_status'] == 1) || ($userOffer['offer_status'] == 3)){?><a href="<?php echo osc_base_url(true).'?page=custom&file=offer_button/offer_edit.php&offer-action=pending&offer-id='.$userOffer['id'] .'&user-id=' . $userOffer['user_id']; ?>"><?php }?><?php _e('Pending', 'offer_button'); ?><?php if(($userOffer['offer_status'] == 1) || ($userOffer['offer_status'] == 3)){?></a><?php }?> | <?php if(($userOffer['offer_status'] == 1) || ($userOffer['offer_status'] == 2)){?><a href="<?php echo osc_base_url(true).'?page=custom&file=offer_button/offer_edit.php&offer-action=declined&offer-id='.$userOffer['id'] .'&user-id=' . $userOffer['user_id']; ?>"><?php }?><?php _e('Decline', 'offer_button'); ?><?php if(($userOffer['offer_status'] == 1) || ($userOffer['offer_status'] == 2)){?></a><?php }?></div></td>
